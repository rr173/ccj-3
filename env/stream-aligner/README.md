...
| 状态 | 含义 |
|---|---|
| `OPEN`（开着）| 还在收窗：有业务窗（存在有效事件的窗）没进单——要么到点未出结果（`missing`），要么窗还没到点（`pending`） |
| `WAITING`（等着）| 窗都齐了，但有的窗还有单边缺口（`unmatched_*` 非空），在等对面 |
| `CLOSED`（关了）| 各窗都齐、没有还在等对面的缺口——成功单 |
| `REOPENED`（被重开）| 关过之后又被订正/撤回/新窗打开，当前不满足关单条件；再次满足时重新 `CLOSED` |
| `VOID`（作废）| 整笔撤空（所有窗都被撤回），不是成功单；之后再来新结果仍是**同一张单**继续出新版本，作废记录永留历史 |

关单条件（缺一不可）：至少一个存活窗；该 key 所有**有有效事件**的窗都已进单（中间没有
事件的空窗不算这笔业务的一部分，不挡关单；但它一旦来了事件/结果就会触发重开）；所有
存活窗都没有未配对单边。被撤回的窗是"已了结"而不是缺口，不挡关单；全部撤回才是作废。
