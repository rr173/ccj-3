...
| `AHEAD_UNCONFIRMED` | 它报到了一个**我还在重试**的版本（投递是至少一次，响应丢了我在重发，它其实已入账）——不扣留任何东西，等我这次重试成功自然回到对齐 |
...
| 没送出过的版本不算 | 它报一个 deliveries 里根本没有的版本（从没给它出过的，或放行前压在内部的），申报被拒（`409`）、台账不动——但这次拒报本身会留痕 |
...
| 方法/路径 | 说明 |
|---|---|
| `POST /postings` | 下游申报入账版本：`{"subscriber_name" 或 "subscriber_id", "window_start", "key", "version"}`；报没送出过的版本返回 `409` 且不计入台账 |
...
| 报了没送出过的版本 | 申报被拒（409），台账不动；这次拒报本身留在台账流水里 |
...
| `POST /postings` | 下游申报某一笔已入账的版本 `{"subscriber_name" 或 "subscriber_id", "window_start", "key", "version"}`；报一个从没给它送出过的版本返回 `409`，这次申报不算数（但留痕） |
