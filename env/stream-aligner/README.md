...
```
  ...
  事件源 A ───▶ │  ingest-a    │                │
                ...
                └──────┬───────┘
                       │  POST /postings 自己申报"入到了哪一版"
                       ▼
                入账台账：对齐 / 落后（扣住这一笔后续版本）/ 对上了一个还在重试的版本
```
...
入完账后下游用 `POST /postings` 自己申报"这一笔入到了哪一版"——送达不等于入账，
对账见下文"下游入账台账"。
...
投递是 at-least-once：**送出去了不等于它入上了**。台账把"入账"的确认权交给下游自己：
它申报某一笔结果入到了哪一版，系统拿这个数和**已确认送达**的版本对账，按
（下游 × 结果）记一本账。

| 概念 | 定义 |
|---|---|
| 它报到哪一版 | `reported_version`：下游最近一次申报已入账的版本（从没报过 = null） |
| 我送到哪一版 | `delivered_up_to`：已确认 `DELIVERED` 的最大版本（还在重试的不算送到） |
| `ALIGNED`（对齐）| 报到版 == 送到版：两边账平 |
| `LAGGING`（落后）| 报到版 < 送到版：它还没入到我已经送到的位置 |
| `AHEAD_UNCONFIRMED` | 它报到了一个**我还在重试**的版本（已发出、响应丢了还没确认送达；投递是至少一次，它其实已入账）——不扣留任何东西，等我这次重试成功自然回到对齐 |

规则：

| 规则 | 行为 |
|---|---|
| 申报才算数 | 只有下游申报了才算入上；光送达不记为对齐。某一笔从没被申报过，台账里 `status`/`reported_version` 为 null，**投递完全不受限**（与建台账之前行为一致）——闸门只从它开始申报的那一笔起生效 |
| 对齐后又送新版 | 新版本的投递一成功，这一笔立刻变 `LAGGING`，绝不"还显示对齐" |
| 落后就扣这一笔 | `LAGGING` 期间这一笔（这个下游 × 这个窗口 × 这个 key）的后续版本**先不送**，等它申报追上 `delivered_up_to` 才自动放行；同下游的**别的笔**（别的 key、同 key 别的窗口）照送不误，绝不被这一笔拖住 |
| 没送到的版本不算 | 它报一个**从没真正发出**的版本——deliveries 里根本没有的，或还扣着没发出的（`PENDING`，比如正被闸门扣住）——申报被拒（`409`）、台账不动，但这次拒报本身会留痕。只有确认送到（`DELIVERED`）或已发出还在重试（`RETRYING`，响应丢了它可能真入上了）的版本，申报才算数 |
| 回滚申报 | 它报一个更早的版本 = 它说自己回滚了：台账跟着退成 `LAGGING`，**已送出的投递记录一条不抹**；它重新追平之前，这一笔后续版本同样扣住 |
| 换地址不换下游 | 台账挂在 subscriber 上（不是 URL）：同名重新登记换了接收地址，账还记在原下游头上，id 不变、历史不断 |

留痕（`posting_events`，只增不改）：每次申报（**含被拒的**）和每次推进
`delivered_up_to` 的投递都落一行，带事件发生前后的完整台账状态。能直接回答
"它什么时候从对齐变成落后、是哪一版把它打落后的"——找
`event=DELIVERY_ADVANCED 且 prev_status=ALIGNED、status=LAGGING` 的行，
`cause_version` 就是那一版，`created_at` 就是那一刻；当前数再变，当时的样子也
不会被盖掉。

| 方法/路径 | 说明 |
|---|---|
| `POST /postings` | 下游申报入账版本：`{"subscriber_name" 或 "subscriber_id", "window_start", "key", "version"}`；报一个没送到的版本（从没发出，或还扣着未投）返回 `409` 且不计入台账 |
...
| 下游申报入账版本 | 台账按（下游 × 结果）记下它报到的版本与我送到的版本：对齐 / 落后 / 对上了一个我还在重试的版本 |
| 对齐后又送了新版 | 这一笔立刻变落后，新投递把它打落后的那一刻与版本号写进台账流水，不盖掉当时的样子 |
| 这一笔落后了 | 该笔后续版本先不送给这个下游，等它申报追平自动放行；别的 key、同 key 别的窗口照送不误 |
